...
date.max <- '2020-07-31' #'2020-12-31'#
...
stan_data <- list(
  DAYS = DAYS, WINDOW = 30, TS = 1:DAYS,
  POP = POP, INDIV = 1/POP,
  prior_a = c(1,2),#c(c=1.725482,sigma=.373588),
  prior_c = c(1,2),#c(a=2.736960,b=28.970829),
  prior_b = c(1,2),#c(a=3.622493,b=14.421170),
  prior_d = c(1,2),#c(a=3.622493,b=14.421170),
  prior_test = c(0.25155568403722195, 0.30983778140500917), #.7,
  prior_test_rec = c(0.25155568403722195, 0.30983778140500917),
  prior_deaths = c(10,1), #c(0.25155568403722195, 0.30983778140500917),
  tests = covid_tests$T,
  ...
  init_solution = c(S = (POP-covid_stats$I[1]-covid_stats$R[1]-covid_stats$D[1])
                      / POP,
                    E = saturate(covid_stats$I[1]/3/POP),
                    I = saturate(covid_stats$I[1]*2/3/POP),
                    R = saturate(covid_stats$R[1]/POP),
